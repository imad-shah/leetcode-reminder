...
async def submit(submission: Submission, db: Session = Depends(get_db)):
    ...
    existing = db.query(SubmissionModel).filter(SubmissionModel.problem == submission.problem).first()
...
async def due_problems(limit: int = Query(default=5, ge=1, le=10), db: Session = Depends(get_db)):
    due = db.query(SubmissionModel).filter(SubmissionModel.due <= datetime.today()).all()
    return due[:limit]